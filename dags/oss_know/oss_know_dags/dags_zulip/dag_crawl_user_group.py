...
with DAG(
    ...
    def do_zulip_crawl_user_group(params):
        ...
        from oss_know.libs.zulip_lib import crawl_user_group
        owner = params["owner"]
        repo = params["repo"]
        site = params["site"]
    ...
    for zulip_info in zulip_info_list:
        op_do_zulip_crawl_user_group = PythonOperator(
            task_id=f'do_zulip_crawl_user_group_{zulip_info["owner"]}_{zulip_info["repo"]}',
            python_callable=do_zulip_crawl_user_group,
            op_kwargs={'params': zulip_info}
        )

        op_init_zulip_crawl_user_group >> op_do_zulip_crawl_user_group
