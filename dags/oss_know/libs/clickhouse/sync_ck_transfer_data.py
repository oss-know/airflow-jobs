...
def get_data_from_opensearch(opensearch_index, opensearch_conn_datas, clickhouse_table):
    ...
    if response['hits']['hits']:
        last_check_timpstamp = response['hits']['hits'][0]["_source"]["os_ck"]["last_data"]["updated_at"]
        logger.info(f"上一次的检查点的时间戳是{last_check_timpstamp}")
    else:
        logger.error(f"没找到上一次检查点的任何信息")
