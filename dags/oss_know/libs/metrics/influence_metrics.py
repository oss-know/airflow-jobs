...
class DeveloperRoleMetricRoutineCalculation(MetricRoutineCalculation):
    def calculate_metrics(self):
        ...
        # owner 必须是一个拥有很多个项目的社区
        sql_ = f"""
-- DeveloperRoleMetricRoutine
select toValidUTF8(substring(author_email, 1, 100)) as author_email,
       total_fix_intensity,
       max_fix_intensity,
       multi_project_participation_degree
from (select a.author_email, total_fix_intensity, max_fix_intensity
      from (
               select author_email, count() as commit_count, ln(commit_count + 1) AS total_fix_intensity
               from (select search_key__owner, search_key__repo, author_email, authored_date, b.*
                     from (select *
                           from gits
                           where search_key__owner = '{self.owner}'
               ...
               join (select *
                     from (select author_email,
                                  search_key__owner,
                                  search_key__repo,
                                  count() as commit_count,
                                  ln(commit_count + 1) as max_fix_intensity
                           from (select search_key__owner, search_key__repo, author_email, authored_date, b.*
                                 from (select *
                                       from gits
                                       where search_key__owner = '{self.owner}'
                           ...
                           order by commit_count desc)
                     limit 1 by author_email) as b on a.author_email = b.author_email) as a global
         join (select author_email, count() as repo_count, ln(repo_count + 1) multi_project_participation_degree
               from (select author_email, search_key__owner, search_key__repo
                     ...
                     group by author_email, search_key__owner, search_key__repo)
               group by author_email
               order by repo_count desc) as b on a.author_email = b.author_email
"""
        results = self.clickhouse_client.execute_no_params(sql_)
...
select splitByString('__', owner_repo)[1]      as owner,
       splitByString('__', owner_repo)[2]      as repo,
       user_login,
       pr_count,
       issue_count,
       commit_count,
       pr_count + issue_count + commit_count as total_contribute_count
from (select if(a.user_login != '', a.user_login, b.author__login) as user_login,
      ...
      from (select if(a.user__login != '', a.user__login, b.user__login) as user_login,
            ...
            from (select user__login,
                  ...
                  where search_key__owner = '{self.owner}'
                    ...
                    and user__login not like '%robot%'
                    and user__login not like '%-bot%'
                    and user__login not like '%githubbot%'
               ...
               full join (select author__login,
                          ...
                          where search_key__owner = '{self.owner}'
                            ...
                            and author__login not like '%-bot%'
                            and author__login not like '%githubbot%'
...
order by total_contribute_count desc
    ...
    def save_metrics(self):
        logger.info('saving  Influence Metrics')
        insert_query = '''
            INSERT INTO contribution_graph_data
            (owner, repo, cntrb_login, pr_count, issue_count, commit_count, total_contributions)
