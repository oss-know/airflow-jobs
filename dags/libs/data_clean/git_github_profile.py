...
from ..base_dict.opensearch_index import OPENSEARCH_GIT_RAW, OPENSEARCH_INDEX_GITHUB_COMMITS, OPENSEARCH_GIT_GITHUB_CLEAN

"""有一个问题就是git的commit 和 github上的commit能否对应的上"""
...
def data_clean(owner, repo, opensearch_conn_datas):
    ...
    for commit in git_results:
        ...
        #owner repo message  hexsha author_name author_email committer_name committer_email authored_date authored_timestamp committed_date committed_timestamp files total
        ...
        now_count += 1
        if now_count % 500 == 0:
            success, failed = do_opensearch_bulk(opensearch_client=opensearch_client, bulk_all_data=all_bulk_data, owner=owner, repo=repo)
...
def get_profile(opensearch_client, login):
    user_profile = opensearch_client.search(index="github_profile",
                                            body={
                                                "query": {
                                                    "term": {
                                                        "login.keyword": {
...
def delete_pre(owner, repo, client):
    ...
    response = client.delete_by_query(index="test_index", body=query)
    logger.info(response)


# if __name__ == "__main__":
#     data_clean("fivestarsky", "airflow-jobs")
