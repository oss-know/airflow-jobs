...
def init_sync_github_commits(github_tokens, opensearch_conn_infos, owner, repo, since=None, until=None):
    ...
    for page in range(9999):
        req = get_github_commits(github_tokens_iter, opensearch_conn_infos, owner, page, repo, since, until)

        now_github_commits = req.json()

        if (now_github_commits is not None) and len(now_github_commits) == 0:
            print("get github commits end to break:: {owner}/{repo} page_index:{page}".format(owner=owner, repo=repo, page=page))
            break
...
def get_github_commits(github_tokens_iter, opensearch_conn_infos, owner, page, repo, since, until):
